-- Group completed streams by their actual start in Bangkok time.
...
begin
  ...
  select
    (date_trunc(
      p_grain,
      (ls.actual_start at time zone 'Asia/Bangkok')
    ))::date as bucket,
  ...
  where ls.actual_end is not null
    and ls.actual_start is not null
    and (not p_own_only or ls.is_own_channel = true)
    and (p_from is null or ls.actual_start >= p_from)
    and (p_to is null or ls.actual_start < p_to)
  group by 1
  order by 1;
end;
$$;
